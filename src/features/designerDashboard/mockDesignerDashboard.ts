...
import type { DesignerRequest, DeliverableItem } from '../designerRequests/mockDesignerRequests';
...
export const getActionItems = (requests: DesignerRequest[]) => {
  ...
  const printsCompleted = requests
    ...
    .reduce((total) => total + Math.floor(Math.random() * 50) + 10, 0); // Random prints per design
...
// Helper functions for DeliverablesSection component
export const getRequestWithDeliverables = (requestId: string) => {
  return designerRequests.find(request => request.id === requestId || request.requestId === requestId);
};

export const addDeliverableItem = (requestId: string, deliverableType: string, item: DeliverableItem) => {
  const request = designerRequests.find(r => r.id === requestId || r.requestId === requestId);
  if (request && request.deliverables) {
    // Handle object-based deliverable structure
    switch (deliverableType) {
      case 'stl':
        if (!request.deliverables.stlFiles) request.deliverables.stlFiles = [];
        request.deliverables.stlFiles.push(item);
        break;
      case 'renders':
        if (!request.deliverables.renderPreviews) request.deliverables.renderPreviews = [];
        request.deliverables.renderPreviews.push(item);
        break;
      case 'notes':
        if (!request.deliverables.assemblyNotes) request.deliverables.assemblyNotes = [];
        request.deliverables.assemblyNotes.push(item);
        break;
      case 'source':
        if (!request.deliverables.sourceFiles) request.deliverables.sourceFiles = [];
        request.deliverables.sourceFiles.push(item);
        break;
    }
  }
  return request;
...
export const removeDeliverableItem = (requestId: string, deliverableType: string, itemId: string) => {
  const request = designerRequests.find(r => r.id === requestId || r.requestId === requestId);
  if (request && request.deliverables) {
    // Handle object-based deliverable structure
    switch (deliverableType) {
      case 'stl':
        if (request.deliverables.stlFiles) {
          request.deliverables.stlFiles = request.deliverables.stlFiles.filter((item: DeliverableItem) => item.id !== itemId);
        }
        break;
      case 'renders':
        if (request.deliverables.renderPreviews) {
          request.deliverables.renderPreviews = request.deliverables.renderPreviews.filter((item: DeliverableItem) => item.id !== itemId);
        }
        break;
      case 'notes':
        if (request.deliverables.assemblyNotes) {
          request.deliverables.assemblyNotes = request.deliverables.assemblyNotes.filter((item: DeliverableItem) => item.id !== itemId);
        }
        break;
      case 'source':
        if (request.deliverables.sourceFiles) {
          request.deliverables.sourceFiles = request.deliverables.sourceFiles.filter((item: DeliverableItem) => item.id !== itemId);
        }
        break;
    }
  }
