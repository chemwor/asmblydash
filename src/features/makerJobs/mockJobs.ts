...
export const updateJobStatus = (id: string, status: string): boolean => {
  ...
  if (jobIndex !== -1) {
    ...
    // Add activity entry for status changes
...
export const updateChecklist = (jobId: string, patch: ChecklistPatch): boolean => {
  ...
  if (jobIndex !== -1) {
    ...
    const completedSteps = Object.entries(patch).filter(([, value]) => value === true);
...
/**
 * Add activity entry with full details (UI-only, no DB calls)
 */
export const addActivity = (jobId: string, entry: Omit<ActivityEntry, 'id' | 'timestamp'>): boolean => {
  ...
  if (jobIndex !== -1) {
    ...
    // Create new activity entry with generated ID and timestamp
    const activityEntry: ActivityEntry = {
      id: `${jobId}-${Date.now()}`,
      timestamp: new Date().toISOString(),
      ...entry
...
/**
 * Add a job to the maker's queue (from job board)
 */
export const addJobToQueue = (jobData: Partial<Job>): boolean => {
  try {
    // Generate a new job ID
    const newJobId = `J-${Date.now().toString().slice(-4)}`;

    const newJob: Job = {
      id: newJobId,
      orderId: jobData.orderId || null,
      product: jobData.product || 'Unknown Product',
      thumbnail: jobData.thumbnail || '/images/products/default.jpg',
      qty: jobData.qty || 1,
      material: jobData.material || 'PLA',
      color: jobData.color || 'Default',
      status: 'Queued',
      dueDate: jobData.dueDate || new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
      priority: jobData.priority || 'Standard',
      payout: jobData.payout || 0,
      completedAt: null,
      requiresQcPhotos: jobData.requiresQcPhotos || false,
      productionChecklist: {
        printStarted: false,
        printCompleted: false,
        qcCompleted: false,
      ...
      activity: [{
        id: `${newJobId}-${Date.now()}`,
        status: 'Queued',
        timestamp: new Date().toISOString(),
        note: 'Job added to queue from job board'
      }]
    };
    ...
    console.log(`Added job ${newJobId} to queue:`, newJob);
    return true;
  } catch (error) {
